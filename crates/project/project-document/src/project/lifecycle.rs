...
impl Project {
    ...
    pub(super) fn migrate_gaussian_items(&mut self) {
            ...
            .flat_map(|track| track.items.iter_mut())
        {
            ...
            item.content =
                VideoItemContent::Gaussian(Box::new(shrimply_3dgs_core::GaussianScene {
                    model: scene.model,
                    camera: shrimply_3dgs_core::Camera3d {
                        source: scene.camera.source,
                        projection: match scene.camera.projection {
                            shrimply_scene_3d::CameraProjection::Perspective => {
                                shrimply_3dgs_core::CameraProjection::Perspective
                            }
                            shrimply_scene_3d::CameraProjection::Orthographic => {
                                shrimply_3dgs_core::CameraProjection::Orthographic
                            }
                            shrimply_scene_3d::CameraProjection::Equirectangular => {
                                shrimply_3dgs_core::CameraProjection::Equirectangular
                            }
                            shrimply_scene_3d::CameraProjection::Cylindrical => {
                                shrimply_3dgs_core::CameraProjection::Cylindrical
                            }
                            shrimply_scene_3d::CameraProjection::Fisheye => {
                                shrimply_3dgs_core::CameraProjection::Fisheye
                            }
                        },
        ...
        for track in self.video_tracks.iter().chain(
            ...
            for item in &track.items {
                ...
                for modifier in &item.modifiers {
                    ...
                    let shrimply_visual_modifiers::scene_3d::Scene3dModifierEffect::Object(object) =
                        &**effect
                    else {
                        continue;
                    };
...
fn validate_visual_alpha_masks(item: &VideoItem) -> Result<(), String> {
    ...
    if item
        ...
        && item.modifier_output_kind()? != shrimply_visual_modifiers::VisualKind::Raster
    {
