...
pub(crate) fn handle_timeline_input(
    ...
    if (pointer.middle_down || pointer.middle_released)
    ...
    if pointer.released {
        ...
        let pos = pointer
            ...
            .unwrap_or(fallback);
        if std::mem::take(&mut runtime.suppress_double_click_selection)
            && runtime.scrollbar_at(pos).is_none()
            && let Some((key, duration)) = insert_caption_on_double_click(
                project,
                pos.x.into(),
                pos.y.into(),
                runtime.view,
        ...
        {
            set_timeline_selection(&project.borrow(), selection_state, vec![key], Some(key));
...
fn update_track_button_hover(runtime: &mut Scene, hovered: Option<TrackButtonId>) {
...
fn update_cut_preview(
    ...
    runtime.cut_preview = crate::cutting::preview(
        project,
...
pub fn insert_caption_on_double_click(
    project: &Rc<RefCell<Project>>,
    x: f64,
    y: f64,
    ...
    let item_key = crate::caption_creation::insert(
        &mut project_state,
        glam::DVec2::new(x, y),
        view,
        snap_repository,
