...
pub struct AddressRippleResult {
    pub selection: Vec<ItemAddress>,
    pub shifted_position: Time,
    pub captions: bool,
    pub video: bool,
    pub audio: bool,
}

pub fn ripple_delete_item_addresses(
...
pub fn ripple_trim_item_addresses(
    context: &impl TimelineOperationContext,
    project: &mut Project,
    selected: &[ItemAddress],
    cut: Time,
...
pub fn ripple_remove_time_ranges(
    project: &mut Project,
    ranges: &[(Time, Time)],
...
#[derive(Clone, Copy)]
pub struct RippleInterval {
    start: Time,
    end: Time,
}

pub fn ripple_intervals_from_ranges(ranges: &[(Time, Time)]) -> Vec<RippleInterval> {
...
pub fn remove_ranges_from_caption_tracks(
    project: &mut Project,
    intervals: &[RippleInterval],
...
pub fn remove_ranges_from_video_tracks(
    project: &mut Project,
    intervals: &[RippleInterval],
    next_group_id: &mut u64,
    group_map: &mut HashMap<(u64, usize), u64>,
...
pub fn remove_ranges_from_audio_tracks(
    project: &mut Project,
    intervals: &[RippleInterval],
    next_group_id: &mut u64,
    group_map: &mut HashMap<(u64, usize), u64>,
...
pub fn remove_ranges_from_items<T: OverwriteItem>(
    items: &mut Vec<T>,
    intervals: &[RippleInterval],
    next_group_id: &mut u64,
    group_map: &mut HashMap<(u64, usize), u64>,
...
pub fn kept_segment_index(intervals: &[RippleInterval], time: Time) -> usize {
    intervals
        .iter()
...
pub fn ripple_track_ranges<T: TimeSlice>(
    items: &[T],
    selected_indices: &HashSet<usize>,
) -> Vec<(Time, Time)> {
...
pub fn ripple_track_items<T: TimeSlice>(
    items: &mut Vec<T>,
    selected_indices: &HashSet<usize>,
    intervals: &[RippleInterval],
    shift_limit: Time,
...
pub fn ripple_position_shift(intervals: &[RippleInterval], position: Time) -> Time {
