...
#[derive(Clone, Copy)]
pub enum NewItemTarget {
    Automatic,
    AtY(f64),
}

#[derive(Clone, Copy)]
pub struct TrackFootprintItem {
...
pub struct NewItemGroup {
    pub kind: TrackKind,
    pub footprint: Vec<TrackFootprintItem>,
}

#[derive(Default)]
pub struct NewItemPlacement {
    pub base: usize,
    pub new_tracks: std::ops::Range<usize>,
}

pub fn place_new_items(
    project: &Project,
    items: &NewItemGroup,
...
pub fn place_new_items_at_base(
    project: &Project,
    items: &NewItemGroup,
    base: usize,
    collision_mode: DragCollisionMode,
...
pub fn track_footprint_span(footprint: &[TrackFootprintItem]) -> usize {
    footprint
        .iter()
...
pub fn visual_track_is_obscured(
    tracks: &[VisualTrack],
    track_index: usize,
    start: Time,
    end: Time,
...
pub struct ItemPlacement {
    pub key: super::ItemKey,
    pub target_track_index: usize,
    pub start: Time,
    pub end: Time,
}

pub fn dragged_group_placements(
    project: &Project,
    group: &DraggedGroup,
...
pub fn can_place_dragged_group(project: &Project, group: &DraggedGroup) -> bool {
...
pub fn placements_collide(placements: &[ItemPlacement]) -> bool {
...
pub fn placements_collide_with_project(
    project: &Project,
    group: &DraggedGroup,
...
pub fn new_track_indices(group: &DraggedGroup, kind: TrackKind) -> Vec<usize> {
...
pub fn placement_indicators(placements: &[ItemPlacement]) -> Vec<DragIndicator> {
    placements
        .iter()
...
pub fn add_collision_tracks(project: &Project, group: &mut DraggedGroup) {
    let Some(placements) = dragged_group_placements(project, group) else {
        return;
    };
...
pub fn insert_new_tracks<T: Default>(tracks: &mut Vec<T>, indices: &[usize]) -> Option<()> {
...
pub fn target_existing_track_index(
...
pub fn time_ranges_collide(start: Time, end: Time, other_start: Time, other_end: Time) -> bool {
