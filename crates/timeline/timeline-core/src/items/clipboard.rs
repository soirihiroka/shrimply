...
pub fn copy_items(project: &Project, selected_items: &[ItemAddress]) -> Option<TimelineClipboard> {
...
pub fn ensure_tracks<T: Default>(tracks: &mut Vec<T>, count: usize) {
    if tracks.len() < count {
        tracks.resize_with(count, T::default);
    }
}

pub fn push_moved_selection(selection: &mut Vec<ItemKey>, key: ItemKey) {
...
pub fn selection_intersects_row(selection: TimelineSelection, row: usize) -> bool {
