...
pub fn drag_position(group: &DraggedGroup) -> DragPosition {
    DragPosition {
...
pub fn restore_drag_position(group: &mut DraggedGroup, position: DragPosition) {
...
pub fn resolve_dragged_group(project: &Project, group: &mut DraggedGroup, previous: DragPosition) {
...
pub fn resolve_overwrite_drag(project: &Project, group: &mut DraggedGroup) {
    group.overwrite_indicators.clear();
    group.blocked_indicators.clear();
...
pub fn resolve_block_drag(project: &Project, group: &mut DraggedGroup, previous: DragPosition) {
    group.overwrite_indicators.clear();
    group.blocked_indicators.clear();
...
pub fn resolve_new_track_drag(project: &Project, group: &mut DraggedGroup) {
    group.overwrite_indicators.clear();
    group.blocked_indicators.clear();
    if !can_place_dragged_group(project, group) {
...
pub fn item_natural_end_edges_at_address(project: &Project, address: &ItemAddress) -> Vec<Time> {
...
pub fn item_natural_snap_targets_at_address(project: &Project, address: &ItemAddress) -> Vec<Time> {
...
pub fn item_natural_resize_candidates_at_address(
    project: &Project,
    address: &ItemAddress,
) -> Vec<Time> {
...
pub fn target_track_index(group: &DraggedGroup, item: &DraggedGroupItem) -> Option<usize> {
    let index = item.key.track_index as isize + track_offset(group, item.key.kind);
    (index >= 0).then_some(index as usize)
}

pub fn track_offset(group: &DraggedGroup, kind: TrackKind) -> isize {
    group
        .track_offsets
        .iter()
...
pub fn set_track_offset(group: &mut DraggedGroup, kind: TrackKind, offset: isize) {
...
pub fn set_track_offsets(project: &Project, group: &mut DraggedGroup, track_offset: isize) {
...
pub fn group_track_bounds(group: &DraggedGroup, kind: TrackKind) -> Option<(isize, isize)> {
...
pub fn target_item_times(group: &DraggedGroup, item: &DraggedGroupItem) -> Option<(Time, Time)> {
...
pub fn move_dragged_group(project: &mut Project, group: &DraggedGroup) -> Option<Vec<ItemKey>> {
