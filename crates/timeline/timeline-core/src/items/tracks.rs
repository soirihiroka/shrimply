...
pub fn row_for_track(project: &Project, kind: TrackKind, track_index: usize) -> Option<usize> {
    rows(project)
        .iter()
        .position(|row| row.root_key == Some(TrackKey { kind, track_index }))
}

pub fn row_for_address(project: &Project, address: &TrackAddress) -> Option<usize> {
    rows(project).iter().position(|row| &row.address == address)
}

pub fn projected_row_for_track(
    project: &Project,
    kind: TrackKind,
    track_index: usize,
...
pub fn projected_row_for_virtual_track(
    project: &Project,
    virtual_tracks: &[(TrackKind, usize)],
...
pub fn expanded_rows_before(project: &Project, kind: TrackKind, end: usize) -> usize {
