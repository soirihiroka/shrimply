...
use crate::{DragMode, ITEM_RESIZE_HANDLE_WIDTH, TimelineCursor};

pub const TRANSITION_TOP_STRIP_HEIGHT: f64 = 12.0;
pub const TRANSITION_HANDLE_RADIUS: f64 = 7.0;

#[derive(Clone, Copy)]
pub struct CursorState {
    pub view: TimelineViewState,
    pub drag_mode: DragMode,
    pub resize_edge: Option<ItemEdge>,
    pub folded_drag: Option<crate::folded_sequence::FoldedDragKind>,
    pub item_transition_drag: bool,
    pub clip_transition_center_drag: Option<bool>,
}

pub fn timeline_cursor(project: &Project, state: CursorState, x: f64, y: f64) -> TimelineCursor {
    match state.drag_mode {
        DragMode::ResizeItem => state
            .resize_edge
            .or_else(|| {
                state.folded_drag.and_then(|drag| match drag {
                    crate::folded_sequence::FoldedDragKind::ResizeStart => Some(ItemEdge::Start),
        ...
        DragMode::Transition => {
            if state.clip_transition_center_drag == Some(true) {
                TimelineCursor::ResizeHorizontal
            } else if state.item_transition_drag || state.clip_transition_center_drag.is_some() {
                TimelineCursor::Crosshair
            } else {
                TimelineCursor::Default
            }
        }
        DragMode::None => hit_clip_transition_at(project, state.view, x, y)
            ...
            .or_else(|| {
                hit_transition_at(project, state.view, x, y)
                    .filter(|hit| !matches!(hit.action, TransitionHitAction::Body))
                    .map(|_| TimelineCursor::Crosshair)
            })
            .or_else(|| {
                crate::folded_sequence::hit_projected_item(project, state.view, x, y)
                    .and_then(|hit| {
                        let (item_x, item_width) =
                            crate::drawing::item_rect(hit.start, hit.end, timeline_x(), state.view);
                    ...
                    .or_else(|| {
                        hit_resize_handle_at(project, state.view, x, y, ITEM_RESIZE_HANDLE_WIDTH)
...
pub fn hit_item_at(project: &Project, view: TimelineViewState, x: f64, y: f64) -> Option<ItemKey> {
