...
#[derive(Clone)]
pub struct FoldedDrag {
    pub key: ItemAddress,
    pub target_track: TrackAddress,
    pub kind: FoldedDragKind,
    pub start: Time,
    pub end: Time,
    pub target_start: Time,
    pub target_end: Time,
    pub pointer_offset: Time,
    pub items: Vec<FoldedDragItem>,
...
pub struct ProjectedItemHit {
    pub key: ItemAddress,
    pub start: Time,
    pub end: Time,
}

pub fn hit_folded_item(
    project: &Project,
...
impl FoldedDrag {
    pub fn preview(&self, address: &ItemAddress) -> Option<&FoldedDragItem> {
        self.items.iter().find(|item| &item.key == address)
    }

    pub fn preview_at(&self, address: &ItemAddress) -> Option<&FoldedDragItem> {
    ...
    pub fn set_target_track(
        ...
        let mut targets = Vec::with_capacity(self.items.len());
        for item in &self.items {
            ...
            targets.push(destination_kind_tracks[target_index as usize].clone());
        }
        for (item, target) in self.items.iter_mut().zip(targets) {
            item.target_track = target;
        }
...
pub fn can_apply_drag(project: &Project, drag: &FoldedDrag) -> bool {
    let mut candidate = project.clone();
...
pub fn update_drag(drag: &mut FoldedDrag, target: Time, minimum_duration: Time) {
    match drag.kind {
        FoldedDragKind::Move => {
...
pub fn reference(project: &Project, key: ItemKey) -> Option<SequenceReference> {
    match key.kind {
...
pub struct ProjectedVideoTrack {
    pub row: usize,
    pub sequence_path: Vec<Uuid>,
    pub track_id: Uuid,
    pub items: Vec<ProjectedVideoItem>,
}

pub struct ProjectedVideoItem {
    pub item: VideoItem,
    pub sequence_path: Option<Vec<Uuid>>,
    pub played_range: Option<(Time, Time)>,
}

pub struct ProjectedAudioTrack {
    pub row: usize,
    pub sequence_path: Vec<Uuid>,
    pub track_id: Uuid,
    pub items: Vec<ProjectedAudioItem>,
}

pub struct ProjectedAudioItem {
    pub item: AudioItem,
    pub sequence_path: Option<Vec<Uuid>>,
    pub played_range: Option<(Time, Time)>,
}
...
pub fn expanded(project: &Project, path: &[Uuid]) -> bool {
    project
...
pub fn child_tracks_before(project: &Project, kind: TrackKind, end: usize) -> usize {
    let mut count = 0;
...
pub fn toggle_sequence(
    ...
    drop(project);
    if collapsed {
        let mut selected = crate::selection_state::selected_nested_items(selection);
        selected.retain(|item| !item.sequence_path().starts_with(&path));
        let focused = crate::selection_state::focused_nested_item(selection)
            .filter(|item| selected.contains(item));
        crate::selection_state::set_selected_nested_items(selection, selected, focused);
    }
}
