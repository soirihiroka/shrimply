...
pub fn root_item_drop(
    context: DragDropContext<'_>,
    group: &crate::items::DraggedGroup,
) -> Option<ItemDrop> {
    if group.items.len() != 1 {
        return None;
    }
    let item = &group.items[0];
    let source = selection_state::item_address(context.project, item.key)?;
    ItemDrop::at(
        context,
        &crate::timeline_operation::SequenceTimeline::root(),
        &source,
        crate::items::target_item_times(group, item)?,
    )
}

pub fn update_root_preview(context: DragDropContext<'_>, group: &mut crate::items::DraggedGroup) {
    if let Some(drop) = root_item_drop(context, group)
        && drop.is_nested()
    {
        group.valid_drop = drop.can_apply(context.project);
        group.preview_status = if !group.valid_drop {
            crate::items::DragPreviewStatus::Blocked
        } else {
            match context.collision_mode {
        ...
        let content_y = context.position.y + context.view.scroll_y - RULER_HEIGHT;
...
impl ItemDrop {
    ...
    pub fn target_track(&self) -> Option<&TrackAddress> {
    ...
    pub fn can_apply(&self, project: &Project) -> bool {
        self.clone().apply(&mut project.clone()).is_some()
    }

    pub fn apply(self, project: &mut Project) -> Option<ItemAddress> {
