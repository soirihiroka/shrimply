...
use shrimply_project_document::project::{CanvasSize, SkiaDrawingStrategy};
use shrimply_project_evaluation::{TransformEvaluation, TransformExpressionCache};
use shrimply_visual_core::generated::draw_visual;
...
pub(crate) use shrimply_visual_core::generated::GeneratedVisual;
...
pub struct GeneratedGpuRenderer {
    ...
    optix_denoiser: Option<shrimply_denoise_optix::OptixDenoiser>,
...
impl GeneratedGpuRenderer {
    ...
    pub(crate) fn render_gaussian_3d(
        ...
        width: u32,
        height: u32,
        params: &shrimply_3dgs_core::RenderParams,
        ...
        let mut renderer = match self.gaussian_3d.take() {
            ...
            None => shrimply_3dgs_vulkan::Renderer::new(shrimply_3dgs_vulkan::RenderContext {
    ...
    pub(crate) fn render_background(
        ...
        if let Some((cached_key, frame)) = &self.background_frame
            && cached_key == &key
        {
            shrimply_profiling::increment("Background raster cache / Hit");
            return Ok(frame.clone());
        }
        shrimply_profiling::increment("Background raster cache / Miss");
    ...
    pub(crate) fn render_mesh_flow(
        &mut self,
        context: Arc<CudaContext>,
        stream: &shrimply_gpu_cuda::CudaStream,
        source: &VisualFrame,
        grid_width: u32,
        grid_height: u32,
    ...
    pub(crate) fn render_visual(
        ...
        let _measurement = shrimply_profiling::measure("Generated visual / Total");
        let render_size = canvas_sizes.0;
        let width = render_size.width.max(1);
        let height = render_size.height.max(1);
        let (image, buffer) = {
            let _measurement = shrimply_profiling::measure("Generated visual / Allocate targets");
            (
        ...
        {
            let _measurement = shrimply_profiling::measure("Generated visual / Skia draw and sync");
        ...
        {
            let _measurement =
                shrimply_profiling::measure("Generated visual / Vulkan copy and wait");
        ...
        let _measurement = shrimply_profiling::measure("Generated visual / CUDA import");
    ...
    pub(super) fn release_render_surfaces(&mut self, requested_bytes: u64) -> Result<bool, String> {
        ...
        if let Some(manim) = self.manim.as_mut() {
            ...
            if manim_released {
                shrimply_gpu_cuda_memory::global().note_manim_render_surface_release();
            }
        }
        let retained = self.skia.resource_cache_usage().resource_bytes;
        shrimply_profiling::set_counter("Generated Skia GPU cache bytes", retained as u64);
        if released {
            tracing::warn!(
                requested_bytes,
    ...
    pub(super) fn release_gpu_animation_resources(&mut self) -> bool {
        ...
        if released {
            shrimply_gpu_cuda_memory::global().note_manim_gpu_animation_release();
        }
        released
    }
    ...
    pub(super) fn release_external_gpu_resources(&mut self) -> bool {
        ...
        shrimply_profiling::set_counter("Generated Skia GPU cache bytes", skia_retained as u64);
        released
    }
    ...
    fn import_buffer_to_cuda_allocation(
        ...
        shrimply_profiling::set_counter("Generated Vulkan / CUDA frames retained", imported_frames);
        shrimply_profiling::set_counter("Generated Vulkan / CUDA bytes retained", imported_bytes);
...
struct ImportedVulkanFrame {
    ...
    allocation_size: u64,
    stream: Option<Arc<shrimply_gpu_cuda::CudaStream>>,
...
fn drop_imported_memory(
    ...
    external_memory: usize,
    allocation_size: u64,
    stream: Option<&shrimply_gpu_cuda::CudaStream>,
    external_semaphore: Option<usize>,
) {
