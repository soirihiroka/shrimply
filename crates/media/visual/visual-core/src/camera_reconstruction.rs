...
pub trait AnalysisFrameRenderer {
    fn render_jpeg(
        &mut self,
        project: &Project,
        position: Time,
...
struct JobState {
    ...
    compute_cancellation: shrimply_compute_client::CancellationToken,
}
...
struct Job {
    ...
    cancellation: Arc<AtomicBool>,
    compute_cancellation: shrimply_compute_client::CancellationToken,
    renderer_factory: AnalysisFrameRendererFactory,
}
...
pub fn analyze(
    ...
    let camera_item_id = camera_address.item_id();
    let service = service();
    let compute_cancellation = match shrimply_compute_client::CancellationToken::new(&server_url) {
...
pub fn sample_for_visual(
    project: &Project,
    item: &VideoItem,
    sequence_path: &[Uuid],
    track_id: Uuid,
    position: Time,
) -> Result<Option<ReconstructedCameraSample>, String> {
    let Some(source) = item.tracking_camera_source() else {
        return Ok(None);
    };
    ...
    {
        return Err("tracked camera source visual track is missing".to_string());
    }
    sample_item(item, source, position)
        .map(Some)
        .ok_or_else(|| "tracked camera analysis is unavailable or out of date".to_string())
}
...
fn process_job(job: Job, state: Arc<Mutex<State>>) {
    let camera_item_id = job.camera_address.item_id();
    {
    ...
    match result {
        Ok(track) => {
            ...
            if let Err(error) = store_track(camera_item_id, &track) {
                tracing::error!(
                    %camera_item_id,
            ...
            state.tracks.insert(camera_item_id, track);
            state
                .jobs
                .get_mut(&camera_item_id)
                .expect("camera analysis job disappeared")
...
fn run_analysis(job: &Job, state: &Mutex<State>) -> Result<CachedTrack, String> {
    ...
    let camera_item_id = job.camera_address.item_id();
    let consuming_track_id = job.camera_address.track_id();
    let mut analysis_project = job.project.clone();
    if !job.camera_address.sequence_path().is_empty() {
        analysis_project.video_tracks = job
            .project
            .video_tracks_for_path(job.camera_address.sequence_path())
            .ok_or_else(|| "camera sequence is missing".to_string())?
            .to_vec();
        analysis_project.audio_tracks = job
            .project
            .audio_tracks_for_path(job.camera_address.sequence_path())
            .ok_or_else(|| "camera sequence audio is missing".to_string())?
            .to_vec();
    }
    let consuming_item = analysis_project
        .video_tracks
        .iter()
        .find(|track| track.id == consuming_track_id)
        .and_then(|track| track.items.iter().find(|item| item.id == camera_item_id))
        .ok_or_else(|| "consuming 3D item is missing".to_string())?;
    if consuming_track_id == job.source.track_id {
        return Err("a 3D item cannot track its own visual track".to_string());
    }
    let source_track = analysis_project
        .video_tracks
        .iter()
    ...
    let source_track_address = TrackAddress::Video {
    ...
    let request = shrimply_compute_client::Tracking3dAnalysisRequest::new(
    ...
    shrimply_compute_client::write_tracking_3d_archive_header(archive.as_file_mut(), &request)?;
    let mut renderer = (job.renderer_factory)()
    ...
    for sample_index in 0..frame_count {
        ...
        shrimply_compute_client::write_tracking_3d_archive_frame(
            archive.as_file_mut(),
            sample_index,
    ...
    shrimply_compute_client::analyze_tracking_3d(
        &job.server_url,
        ...
        |event| {
            ...
            match event {
                shrimply_compute_client::Tracking3dEvent::Queued { position } => {
                    let mut state = state.lock().expect("camera reconstruction state poisoned");
                    if let Some(current) = state
                        .jobs
                        .get_mut(&camera_item_id)
                    ...
                    {
                        current.status = AnalysisStatus::Analyzing {
                            message: shrimply_compute_client::queued_status(position),
                            completed_frames: 0,
                            total_frames: 0,
                        };
                    }
                ...
                shrimply_compute_client::Tracking3dEvent::Progress {
                    message,
                    completed_frames,
                    total_frames,
                } => {
                ...
                shrimply_compute_client::Tracking3dEvent::Camera(camera) => {
                    reconstructed.push(camera);
                    true
                }
                shrimply_compute_client::Tracking3dEvent::Result { camera_count } => {
                    result_count = Some(camera_count);
                    true
                }
                shrimply_compute_client::Tracking3dEvent::Error { code, message } => {
