...
struct AffineChunk {
    ...
    source_transforms: Vec<glam::Mat3>,
}
...
pub enum FrameState {
    Disabled,
    Pending,
    Ready(StabilizationWarp),
    Failed(String),
}

#[derive(Clone, Copy)]
enum GenerationOptions {
    #[cfg(feature = "cuda-stabilization")]
    L1(shrimply_affine_l1_stabilization::StabilizationOptions),
    #[cfg(not(feature = "cuda-stabilization"))]
    L1Unavailable,
...
pub fn source_warp(item: &VideoItem, source_position: Time) -> Option<StabilizationWarp> {
    source_warp_result(item, source_position).ok().flatten()
}

pub fn source_warp_result(
    item: &VideoItem,
    source_position: Time,
) -> Result<Option<StabilizationWarp>, String> {
    match frame_state(item, source_position) {
        FrameState::Disabled | FrameState::Pending => Ok(None),
        FrameState::Ready(warp) => Ok(Some(warp)),
        FrameState::Failed(error) => Err(error),
    }
}

pub fn frame_state(item: &VideoItem, source_position: Time) -> FrameState {
    if !eligible(item) {
        return FrameState::Disabled;
    }
    if let Err(error) = ensure_available(item) {
        return FrameState::Failed(error);
    }
    let key = match cache_key(item, source_position) {
        Ok(key) => key,
        Err(error) => return FrameState::Failed(error),
    };
    ...
    let Some(chunk) = cached(&key) else {
        request_at(item, source_position);
        return FrameState::Pending;
    };
    let warp = match chunk.as_ref() {
        StabilizationChunk::L1(chunk) => {
            ...
            let Some(index) = frame.checked_sub(chunk.first_frame) else {
                return FrameState::Failed(
                    "Stabilization chunk does not cover the decoded frame".into(),
                );
            };
            chunk
                .source_transforms
        ...
        StabilizationChunk::MeshFlow(chunk) => {
            ...
            chunk
                .source_offsets
                .get(index as usize)
                .map(|source_offsets| StabilizationWarp::Mesh {
    ...
    warp.map(FrameState::Ready).unwrap_or_else(|| {
        FrameState::Failed("Stabilization chunk has no warp for the decoded frame".into())
    })
}
...
pub fn has_failed(item: &VideoItem) -> bool {
    if ensure_available(item).is_err() {
        return true;
    }
    cache_base(item).is_ok_and(|base| {
        FAILED
...
fn generate(
    ...
    let chunk = match options {
        ...
        GenerationOptions::L1(options) => {
            ...
            StabilizationChunk::L1(AffineChunk {
                first_frame: chunk.first_frame,
                frame_rate_numerator: chunk.frame_rate_numerator,
                frame_rate_denominator: chunk.frame_rate_denominator,
                source_transforms: chunk.source_transforms,
            })
        }
        #[cfg(not(feature = "cuda-stabilization"))]
        GenerationOptions::L1Unavailable => {
            return Err("L1 video stabilization requires the CUDA analyzer".into());
        }
...
fn ensure_available(_item: &VideoItem) -> Result<(), String> {
    #[cfg(not(feature = "cuda-stabilization"))]
    if matches!(_item.stabilization_method(), VideoStabilizationMethod::L1) {
        return Err(
            "L1 video stabilization requires the CUDA analyzer and is unavailable on Metal".into(),
        );
    }
    Ok(())
}
...
fn initial_position(item: &VideoItem) -> Time {
    shrimply_project_document::project::video_source_time_at(item, item.start)
    ...
    fn load(&self, key: &CacheKey) -> Result<Option<StabilizationChunk>, String> {
        ...
        if key.base.method == 1 {
            ...
            let source_transforms = bytes
                ...
                .collect();
            return Ok(Some(StabilizationChunk::L1(AffineChunk {
