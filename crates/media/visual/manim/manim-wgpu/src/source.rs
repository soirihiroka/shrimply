...
use shrimply_manim_bridge::{Progress, Settings, compile, reflected_parameters};
use shrimply_manim_ir::CompiledAnimation;
use shrimply_manim_state::{SourceIdentity, Update};
...
impl Source {
    ...
    pub fn poll(
        ...
        let source_time =
            shrimply_project_document::project::generated_source_time_at(item, position)
...
pub fn effective_fps(item: &VideoItem, project_fps: Fraction) -> Fraction {
    if item.playback_fps == shrimply_project_document::project::native_playback_fps() {
        project_fps
    } else {
        item.playback_fps
    }
...
pub fn loading_pixels(
    ...
    shrimply_loading_screen_skia::render(
        width,
        height,
        &counter,
        LOADING_COLOR,
