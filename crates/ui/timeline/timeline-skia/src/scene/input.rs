    ...
    pub fn animating(&self) -> bool {
        ...
        !self.pending_scrolls.is_empty()
            ...
            || self.overscroll.is_some_and(|overscroll| {
                shrimply_components_skia::overshoot_distance(
                    overscroll.distance,
                    overscroll.started_at.elapsed(),
                ) > shrimply_components_skia::OVERSHOOT_VISIBLE_DISTANCE
            })
    }
    ...
    pub fn pointer_exited(&mut self) {
        ...
        if let Some(id) = self.hovered_track_button.take() {
            self.track_buttons
                .entry(id)
                .or_default()
                .event(shrimply_components_skia::button::Event::PointerLeft);
        }
    }
    ...
    pub fn pointer_cancelled(&mut self) {
        ...
        .flatten()
        {
            self.track_buttons
                .entry(id)
                .or_default()
                .event(shrimply_components_skia::button::Event::Cancelled);
        }
