...
use shrimply_audio_engine::{AudioPlayer, SharedAudioLevels};
use shrimply_components_skia::canvas::UVec2;
use shrimply_components_skia::canvas::{Rect, vec2};
...
use shrimply_preview_runtime_cuda::captions::{CaptionAppearance, draw_captions};
use shrimply_preview_runtime_cuda::guides;
use shrimply_preview_runtime_cuda::preferences::store as preferences_store;
use shrimply_preview_runtime_cuda::renderer::{Appearance, VideoRenderer};
use shrimply_preview_runtime_cuda::{PreviewMedia, StepDirection, rendered_frame_rate_label};
use shrimply_project_document::project::{Project, Time};
use shrimply_surface_gl_skia::GlAudioMeter;
use shrimply_visual_cuda::compositor::{
    CompositeAccuracy, VideoCommand, VideoCommandSender, VideoEvent,
};
use shrimply_visual_cuda::gpu::CompositedVideoFrame;
...
struct Surfaces {
    ...
    track_add_kind: shrimply_timeline_edit::TrackKind,
    track_add_x: f32,
    track_add_y: f32,
...
pub fn install(session: &EditorSession) -> Result<(), String> {
    ...
    SURFACES.with_borrow_mut(|surfaces| {
        ...
        *surfaces = Some(Surfaces {
            ...
            track_add_kind: shrimply_timeline_edit::TrackKind::Video,
            track_add_x: 0.0,
            track_add_y: 0.0,
