...
mod tests {
    ...
    #[test]
    fn case_7_font_variations_serialized_at_default_and_reset() {
        let (controller, project, target, _) = test_controller();
        let (serialized, _) = controller
            .control_graph_source(&target)
            .expect("target must serialize");

        assert_eq!(
            serialized.pointer("/content/font_variations"),
...

        {
            ...
            let video_item = p.video_item_mut(address).expect("video item");
            let VideoItemContent::Text(text) = &mut video_item.content else {
                panic!("expected text item");
            };
            text.font_variations.push(FontVariation {
                axis: "wght".to_string(),
        ...
        controller
            .reset_video(&target, &reset)
            .expect("resetting appearance card with non-empty font_variations must succeed");

        let p = project.borrow();
        let InspectorTarget::Item(address) = &target else {
            panic!("expected item target");
        };
        let ItemRef::Video(video_item) = p.item(address).expect("video item");
        let VideoItemContent::Text(text) = &video_item.content else {
            panic!("expected text item");
        };
        assert!(text.font_variations.is_empty());
    }
}
