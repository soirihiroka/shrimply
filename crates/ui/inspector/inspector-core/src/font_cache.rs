...
use shrimply_project_document::project::{
    FontFamily as ProjectFontFamily, Project, VideoItemContent,
};
...
fn project_google_fonts(project: &Project, default_font: &ProjectFontFamily) -> HashSet<String> {
        ...
        .flat_map(|track| &track.items)
    {
        ...
        for modifier in &item.modifiers {
            let shrimply_visual_modifiers::ModifierEffect::Scene3d(effect) = &modifier.effect
            else {
                continue;
            };
            if let shrimply_visual_modifiers::scene_3d::Scene3dModifierEffect::Text(text) =
                &**effect
            {
                add(&text.font_families);
            }
...
pub fn activate_family(family: MaterializedFamily) -> Result<(), String> {
    ...
    drop(active);
    for path in paths {
        shrimply_visual_core::text_layout::register_application_font(path)?;
    }
    Ok(())
}
