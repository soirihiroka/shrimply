...
pub struct InspectorRuntime {
    ...
    pub frame_rate: shrimply_property_model::timeline_value::Fraction,
}
...
pub type CameraAnalysis =
    fn(Project, ItemAddress, shrimply_3dgs_core::TrackingCameraSource, String);
pub type TransparentFillAnalysis =
    fn(
        Project,
        &ItemAddress,
        uuid::Uuid,
    ) -> Result<shrimply_visual_core::transparent_fill::analysis::RunId, String>;
...
impl Default for InspectorAnalysisBackend {
    fn default() -> Self {
        Self {
            camera: Some(shrimply_visual_cuda::camera_reconstruction::analyze),
            transparent_fill: Some(shrimply_visual_cuda::transparent_fill_analysis::analyze),
            visual_cache: Some(shrimply_visual_cuda::modifier_cache::bake),
        }
    }
}
...
pub struct InspectorController {
    ...
    default_text_font: Option<shrimply_property_model::FontFamily>,
    ...
    pub(crate) expression_cache: Rc<RefCell<shrimply_project_evaluation::TransformExpressionCache>>,
    pub(crate) audio_sampler: Rc<RefCell<shrimply_audio_engine::streaming::FrameAudioSampler>>,
...
pub(crate) struct CachedTransparentFillStatus {
    ...
    pub(crate) prepared: shrimply_visual_core::transparent_fill::analysis::PreparedStatus,
}
...
impl InspectorController {
    pub fn new(
        ...
        Self {
            ...
            audio_sampler: Rc::new(RefCell::new(
                shrimply_audio_engine::streaming::FrameAudioSampler::preview(
                    shrimply_audio_engine::streaming::EXPRESSION_SAMPLE_RATE_HZ,
                ),
            )),
    ...
    pub fn with_default_text_font(mut self, font: shrimply_property_model::FontFamily) -> Self {
    ...
    pub fn audio_sampler(
        &self,
    ) -> Rc<RefCell<shrimply_audio_engine::streaming::FrameAudioSampler>> {
        self.audio_sampler.clone()
    }

    pub fn audio_analysis_at(
        &self,
        position: Time,
    ...
    pub fn resolve_preview_focus(
    ...
    ) -> Option<shrimply_editor_state::preview_focus::FocusedPreview> {
        ...
        item.owns_preview_target(resolved).then(|| {
    ...
    pub fn apply_project_settings(
        &self,
        canvas_size: shrimply_project_document::project::CanvasSize,
        ...
        shrimply_project_document::project::commit_edit(&project, "project-settings");
        drop(project);
        player_state::refresh_project(
            &self.player_state,
            player_state::ProjectChange {
    ...
    pub(crate) fn edit_value_if_changed_with_refresh_and_commit(
        ...
        match commit {
            ...
            InspectorCommit::Coalesced(name) => {
                shrimply_project_document::project::commit_coalesced_edit(&project, name);
            }
            InspectorCommit::Immediate(name) => {
                shrimply_project_document::project::commit_edit(&project, name);
            }
        }
...
fn target_details(
    ...
    match target {
        InspectorTarget::Project => vec![
            ...
            InspectorDetail {
                label: "Duration",
                value: shrimply_project_document::time_format::project_duration(project.duration()),
            },
            InspectorDetail {
                label: "Project File",
                value: shrimply_project_document::project::active_project_path()
                    .to_string_lossy()
                    .into_owned(),
            },
        ],
        ...
        InspectorTarget::Item(address) => {
            ...
            if let Some((timeline_start, timeline_end)) = project.projected_item_times(address) {
                details.push(InspectorDetail {
                    ...
                    value: shrimply_project_document::time_format::playback_time(timeline_start),
                });
                details.push(InspectorDetail {
                    label: "Timeline End",
                    value: shrimply_project_document::time_format::playback_time(timeline_end),
                });
            }
            details.push(InspectorDetail {
                label: "Local Start",
                value: shrimply_project_document::time_format::playback_time(start),
            });
            details.push(InspectorDetail {
                label: "Local End",
                value: shrimply_project_document::time_format::playback_time(end),
            });
            if let Some(natural_duration) = natural_duration {
                details.push(InspectorDetail {
                    label: "Natural Duration",
                    value: shrimply_project_document::time_format::playback_time(natural_duration),
                });
            }
            details.push(InspectorDetail {
                label: "Timeline Duration",
                value: shrimply_project_document::time_format::playback_time(
            ...
            if let Some(source_offset) = source_offset {
                details.push(InspectorDetail {
                    label: "Source Offset",
                    value: shrimply_project_document::time_format::playback_time(source_offset),
                });
            }
...
pub(crate) fn video_title(item: &shrimply_project_document::project::VideoItem) -> &'static str {
