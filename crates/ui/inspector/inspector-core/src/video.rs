...
use shrimply_property_model::timeline_value::{TimelineBase, TimelineBool, TimelineValue};
use shrimply_visual_modifiers::VisualKind;
...
pub struct BlenderSourcePresentation {
    pub item: shrimply_project_document::project::BlenderItem,
    pub asset: String,
}
...
pub struct VideoCard {
    ...
    pub preview_facet: Option<shrimply_preview_provider_skia::PreviewFacetKey>,
...
impl VideoCard {
    ...
    pub(crate) fn preview_facet(
        mut self,
        facet: shrimply_preview_provider_skia::PreviewFacetKey,
    ) -> Self {
...
impl InspectorController {
    pub fn reset_video(&self, target: &InspectorTarget, reset: &VideoReset) -> Result<(), String> {
        ...
        if let Some((path, value)) = &reset.fraction {
            ...
            shrimply_project_document::project::commit_edit(&project, reset.commit_name);
            drop(project);
        ...
        if reset.cancel_stabilization {
            shrimply_visual_core::stabilization::cancel(
                project
                    .video_item(address)
        ...
        *current = replacement;
        ...
        if stabilization_setting(path) {
            ...
            if item.stabilize_video {
                shrimply_visual_core::stabilization::request(item);
            }
        }
        if commit_immediately {
            shrimply_project_document::project::commit_edit(&project, commit_name);
        ...
        match action {
            ...
            InspectorControlAction::ToggleSam2Analysis {
                ...
                Ok(Some(crate::visual_modifiers::sam2_analysis_control(
                    &shrimply_visual_core::sam2::analysis::AnalysisTarget {
        ...
        match action {
            InspectorControlAction::RebuildVideoStabilization => {
                ...
                let item = project
                    .video_item(address)
                    .ok_or_else(|| "video item is no longer available".to_string())?;
                if shrimply_visual_core::stabilization::is_generating(item) {
                    shrimply_visual_core::stabilization::cancel(item);
                } else {
                    let timeline_position = player_state::current_time(&self.player_state);
                    let source_position = shrimply_project_document::project::video_source_time_at(
                        item,
                        timeline_position,
                    )
                    .unwrap_or(item.time_offset);
                    shrimply_visual_core::stabilization::rebuild(item, source_position);
                }
            }
    ...
    pub fn video_stabilization_generating(&self, target: &InspectorTarget) -> Option<bool> {
        ...
        self.project
            .borrow()
            .video_item(address)
            .map(shrimply_visual_core::stabilization::is_generating)
    }
}
...
fn validate_video_target(target: &InspectorTarget) -> Result<(), String> {
    if matches!(
        target,
        InspectorTarget::Item(shrimply_project_document::project::ItemAddress::Video { .. })
    ) {
        Ok(())
    } else {
            ...
            [
                ...
                (
                    ...
                    serde_json::to_value(TimelineValue::<
                        shrimply_project_document::project::VideoSampleMethod,
                    >::default())
...
fn stabilization_item(item: &VideoItem) -> VideoCard {
    ...
    section.add(
        ...
        .busy(shrimply_visual_core::stabilization::is_generating(item))
    ...
    if method != VideoStabilizationMethod::Off {
        let generating = shrimply_visual_core::stabilization::is_generating(item);
        section.add(
    ...
    VideoCard::new("stabilization", "Stabilization", section).reset_fields(
        [
            ...
            (
                "/stabilization_crop_ratio",
                Value::from(f64::from(
                    shrimply_project_document::project::default_video_stabilization_crop_ratio(),
                )),
            ),
            (
                "/stabilization_first_derivative_weight",
                Value::from(f64::from(
                    shrimply_project_document::project::default_video_stabilization_first_derivative_weight(),
                )),
            ),
            (
                "/stabilization_second_derivative_weight",
                Value::from(f64::from(
                    shrimply_project_document::project::default_video_stabilization_second_derivative_weight(),
                )),
            ),
            (
                "/stabilization_third_derivative_weight",
                Value::from(f64::from(
                    shrimply_project_document::project::default_video_stabilization_third_derivative_weight(),
                )),
            ),
            (
                "/mesh_flow_rows",
                Value::from(shrimply_project_document::project::default_mesh_flow_rows()),
            ),
            (
                "/mesh_flow_columns",
                Value::from(shrimply_project_document::project::default_mesh_flow_columns()),
            ),
            (
                "/mesh_flow_smoothing_radius",
                Value::from(shrimply_project_document::project::default_mesh_flow_smoothing_radius()),
            ),
            (
                "/mesh_flow_iterations",
                Value::from(shrimply_project_document::project::default_mesh_flow_iterations()),
            ),
            (
                "/mesh_flow_adaptive_weights",
                serde_json::to_value(
                    shrimply_project_document::project::MeshFlowAdaptiveWeights::default(),
                )
...
fn stabilization_status(item: &VideoItem) -> &'static str {
    if item.alpha_mask_video.is_some() {
        ...
        ""
    } else if shrimply_visual_core::stabilization::is_generating(item) {
        "Analyzing source motion…"
    } else if shrimply_visual_core::stabilization::has_failed(item) {
        "Analysis failed; use Rebuild to retry"
    } else if shrimply_visual_core::stabilization::is_ready(item) {
