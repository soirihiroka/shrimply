...
pub fn presentation(item: &VideoItem) -> Option<ManimPresentation> {
    ...
    let reflected = shrimply_editor_state::manim_status::parameters(
        item.id,
        source_revision,
        &manim.scene,
        &manim.parameters,
    ...
    if let Some(error) = shrimply_editor_state::manim_status::error(
        item.id,
        source_revision,
        &manim.scene,
        &manim.parameters,
...
pub fn scenes(source: &Asset, current: &str) -> ManimScenes {
    ...
    if cache.pending.insert(key.clone()) {
        ...
        std::thread::spawn(move || {
            let result = shrimply_manim_bridge::discover_scenes(snapshot.asset());
...
pub fn reload_source(source: &Asset) -> Result<(), String> {
    let invalidate = shrimply_manim_bridge::invalidate_ir_cache(source);
...
fn manim_mut<'a>(
    project: &'a mut shrimply_project_document::project::Project,
    target: &InspectorTarget,
...
fn refresh(player: &shrimply_editor_state::player_state::SharedPlayerState, inspector: bool) {
