...
pub use shrimply_components_skia::slider::ScrollInput as FrameGraphScrollInput;
...
impl FrameGraphState {
    pub fn new(
        ...
        let minimum_seconds_per_pixel = matches!(graph, KeyframeGraph::Step { .. }).then(|| {
            frame_step.as_secs_f64() / shrimply_discrete_keyframe_graph_skia::MAX_FRAME_WIDTH
        });
        Self {
            graph,
            item_range,
    ...
    pub fn draw(&mut self, painter: &TimelinePainter, width: f64, height: f64) {
        ...
        let overscroll = self.overscroll.and_then(|overscroll| {
            let distance = shrimply_components_skia::overshoot_distance(
                overscroll.distance,
                overscroll.started_at.elapsed(),
            );
            (distance > shrimply_components_skia::OVERSHOOT_VISIBLE_DISTANCE)
    ...
    pub fn preferred_height(&self) -> i32 {
        if matches!(self.graph, KeyframeGraph::Step { .. }) {
            shrimply_discrete_keyframe_graph_skia::CONTENT_HEIGHT + GRAPH_SLIDER_HEIGHT as i32
        } else {
            FRAME_GRAPH_HEIGHT
        }
    }
        ...
        self.view.minimum_seconds_per_pixel =
            matches!(graph, KeyframeGraph::Step { .. }).then(|| {
                self.frame_step.as_secs_f64()
                    / shrimply_discrete_keyframe_graph_skia::MAX_FRAME_WIDTH
            });
        self.graph = graph;
        ...
        match drag.target {
            ...
            DragTarget::Point(focus_time) => {
                ...
                let point_x = if matches!(self.graph, KeyframeGraph::Step { .. }) {
                    x - shrimply_discrete_keyframe_graph_skia::frame_width(
                        width,
                        domain,
                        self.frame_step,
                    ) / 2.0
    ...
    fn set_scroll(&mut self, width: f64, target: f64) {
        ...
        self.overscroll = edge.map(|(edge, distance)| GraphOverscroll {
            ...
            distance: (distance / self.view.seconds_per_pixel)
                .clamp(1.0, shrimply_components_skia::OVERSHOOT_MAX_DISTANCE),
        });
    }
    ...
    fn scrollbar(&self, width: f64, height: f64) -> Option<Scrollbar> {
        ...
        Some(Scrollbar {
            ...
            bounds: shrimply_components_skia::Rect::from_xywh(
                0.0,
...
fn draw_selection_box(
    ...
    use shrimply_components_skia::canvas::{Rect, Stroke, StrokeKind, vec2};
